...
// --- VTpass Configuration and Helper Function ---

const vtpassConfig = {
  apiKey: process.env.VTPASS_API_KEY,
  secretKey: process.env.VTPASS_SECRET_KEY,
  baseUrl: 'https://sandbox.vtpass.com/api',
  virtualAccountServiceId: process.env.VTPASS_VIRTUAL_ACCOUNT_SERVICE_ID,
};
...
app.post('/api/users/virtual-account/generate', protect, async (req, res) => {
  ...
  try {
    ...
    if (user.virtualAccount.assigned) {
      return res.status(400).json({ success: false, message: 'User already has a virtual account assigned.' });
    }
    // Split full name into first and last names for the API call
    const nameParts = user.fullName.split(' ');
    const firstName = nameParts[0];
    const lastName = nameParts.length > 1 ? nameParts.slice(1).join(' ') : nameParts[0];

    const vtpassResult = await callVtpassApi('/service/dedicated/account/create', {
      email: user.email,
      first_name: firstName,
      last_name: lastName,
      phone: user.phone,
      serviceID: vtpassConfig.virtualAccountServiceId,
    });

    if (vtpassResult.success && vtpassResult.data.code === '000') {
      const { bank_name, account_number, account_name } = vtpassResult.data.content;
      user.virtualAccount = {
        assigned: true,
        bankName: bank_name,
      ...
      await user.save();
      res.json({ success: true, message: 'Virtual account created successfully.', virtualAccount: user.virtualAccount });
    } else {
      res.status(vtpassResult.status || 400).json(vtpassResult);
    }
  } catch (err) {
...
app.get('/api/users/virtual-account/:userId', protect, async (req, res) => {
  ...
  try {
    ...
    if (!user.virtualAccount.assigned) {
      return res.status(404).json({ success: false, message: 'Virtual account not yet assigned.' });
    }
    res.json({ success: true, virtualAccount: user.virtualAccount });
  } catch (err) {
    console.error('Error fetching virtual account details:', err);
    res.status(500).json({ success: false, message: 'Internal Server Error' });
  }
});
...
// NEW: Get service variations (e.g., data plans) for a serviceID
app.get('/api/vtpass/service-variations/:serviceID', protect, async (req, res) => {
  ...
  try {
    const response = await axios.get(`${vtpassConfig.baseUrl}/service-variations?serviceID=${serviceID}`, {
      headers: {
        'api-key': vtpassConfig.apiKey,
        'secret-key': vtpassConfig.secretKey,
      }
    ...
    if (response.data.response_code === '000') {
      res.json({ success: true, message: 'Service variations retrieved successfully.', data: response.data.content.variations });
    } else {
      res.status(400).json({ success: false, message: response.data.response_description || 'Failed to retrieve service variations.', details: response.data });
    }
  } catch (error) {
...
app.get('/api/vtpass/status/:reference', protect, async (req, res) => {
  const { reference } = req.params;

  try {
    const response = await axios.post(`${vtpassConfig.baseUrl}/requery`, { request_id: reference }, {
      headers: {
        'api-key': vtpassConfig.apiKey,
        'secret-key': vtpassConfig.secretKey,
        'Content-Type': 'application/json'
    ...
    if (response.data.response_code === '000') {
      res.json({ success: true, message: 'Transaction status retrieved successfully.', data: response.data.content });
    } else {
      res.status(400).json({ success: false, message: response.data.response_description || 'Failed to retrieve transaction status.', details: response.data });
    }
  } catch (error) {
...
// Airtime Purchase (Updated logic for pending status)
app.post('/api/vtpass/airtime/purchase', protect, async (req, res) => {
  ...
  try {
    ...
    // A VTpass response_code of '00' means successful. '01' means pending.
    if (vtpassResult.success) {
      if (vtpassResult.data.response_code === '00') {
        transactionStatus = 'successful';
        // Only deduct from wallet if transaction is successful
        newBalance = user.walletBalance - amount;
        user.walletBalance = newBalance;
        await user.save({ session });
      } else if (vtpassResult.data.response_code === '01') {
        transactionStatus = 'pending';
        // Do NOT deduct from wallet for pending transactions. This avoids a failed deduction.
        // The user will be debited later if the transaction becomes successful.
        // You would need a webhook or a separate system to check for status updates and credit/debit the user accordingly.
        // For now, we simply record the pending status.
      } else {
        await session.abortTransaction();
        session.endSession();
      ...
      // If the VTpass API call itself failed (e.g., timeout), abort.
      await session.abortTransaction();
      session.endSession();
      return res.status(vtpassResult.status || 400).json(vtpassResult);
    }
    ...
    const newTransaction = new Transaction({
      ...
      balanceBefore,
      balanceAfter: newBalance, // Reflect the updated balance if successful, or old balance if pending
      reference,
    });
    await newTransaction.save({ session });
...
// Data Purchase (Updated logic for pending status)
...
// Cable TV Payment (Updated logic for pending status)
