...
app.post('/api/payments/sync-payment', async (req, res) => {
  try {
    const { reference, amount, userId, source } = req.body;
    
    console.log('🔄 SYNC: Syncing payment to main backend:', { reference, amount, userId });

    // Check if transaction already exists
    const existingTransaction = await Transaction.findOne({ reference });
    if (existingTransaction) {
      console.log('⚠️ SYNC: Transaction already exists:', reference);
    ...
    const transaction = new Transaction({
      ...
      type: 'deposit',
      status: 'success',
      gateway: source || 'paystack',
      description: `Wallet top-up via ${source || 'paystack'}`,
      createdAt: new Date()
    });

    await transaction.save();
    console.log('✅ SYNC: Transaction saved:', reference);
    ...
    if (user) {
      ...
      console.log('✅ SYNC: User balance updated:', { 
        userId, 
        oldBalance, 
        newBalance: user.walletBalance 
      });
    ...
    console.error('❌ SYNC: Sync payment error:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Failed to sync payment: ' + error.message 
    });
  }
});
