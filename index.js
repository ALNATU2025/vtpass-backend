...
// ==================== ADMIN SERVICE COMMISSION STATISTICS - FULL VERSION ====================
...
app.get('/api/admin/commission-stats', adminProtect, async (req, res) => {
  try {
    ...
    const serviceCommissionRates = {
      'airtime': { rate: 0.02, label: 'Airtime', type: 'percentage', icon: '📱' },
      'international_airtime': { rate: 0.02, label: 'International Airtime', type: 'percentage', icon: '🌍' },
      'cable': { rate: 0.013, label: 'Cable TV', type: 'percentage', icon: '📺' },
      'data': { rate: 0.015, label: 'Data', type: 'percentage', icon: '📶' },
      'electricity': { rate: 0.01, label: 'Electricity', type: 'percentage', icon: '⚡' },
      'education': { rate: 100, label: 'Education', isFlat: true, icon: '🎓' },
      'betting': { rate: 0.012, label: 'Betting', type: 'percentage', icon: '🎰' },
      'ticket': { rate: 150, label: 'Ticket', isFlat: true, icon: '🎫' }
    };
    ...
    // ================================================
    // 7. GET PERIOD STATS (FULL DATASET)
    // ================================================
    const todayFilter = { ...baseFilter, createdAt: { $gte: today } };
    const weekFilter = { ...baseFilter, createdAt: { $gte: weekStart } };
    const monthFilter = { ...baseFilter, createdAt: { $gte: monthStart } };
    const yearFilter = { ...baseFilter, createdAt: { $gte: yearStart } };

    const [
      todayCount, todayAmountAgg,
      weekCount, weekAmountAgg,
      monthCount, monthAmountAgg,
      yearCount, yearAmountAgg
    ] = await Promise.all([
      Transaction.countDocuments(todayFilter),
      Transaction.aggregate([{ $match: todayFilter }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
      Transaction.countDocuments(weekFilter),
      Transaction.aggregate([{ $match: weekFilter }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
      Transaction.countDocuments(monthFilter),
      Transaction.aggregate([{ $match: monthFilter }, { $group: { _id: null, total: { $sum: '$amount' } } }]),
      Transaction.countDocuments(yearFilter),
      Transaction.aggregate([{ $match: yearFilter }, { $group: { _id: null, total: { $sum: '$amount' } } }])
    ]);

    const todayAmount = todayAmountAgg[0]?.total || 0;
    const weekAmount = weekAmountAgg[0]?.total || 0;
    const monthAmount = monthAmountAgg[0]?.total || 0;
    const yearAmount = yearAmountAgg[0]?.total || 0;

    // ================================================
    // 8. SERVICE BREAKDOWN BY PERIOD
    // ================================================
    const getServiceBreakdown = async (filter) => {
      const breakdown = {};
      Object.keys(serviceTypeMap).forEach(key => {
        breakdown[key] = {
          label: serviceCommissionRates[key]?.label || key,
      ...
      const serviceTransactions = await Transaction.find({ ...filter, status: { $regex: /^success|completed$/i } })
        .select('type amount')
        .lean();
      ...
      for (const tx of serviceTransactions) {
        ...
        if (breakdown[serviceType]) {
          ...
          breakdown[serviceType].amount += tx.amount || 0;
          ...
          if (rateConfig) {
            if (rateConfig.isFlat) {
              ...
              breakdown[serviceType].commission += (tx.amount || 0) * (rateConfig.rate || 0);
            }
          }
        }
      }
    ...
    // ================================================
    // 9. GET BREAKDOWN FOR EACH PERIOD
    // ================================================
    const [todayBreakdown, weekBreakdown, monthBreakdown, yearBreakdown, allTimeBreakdown] = await Promise.all([
      getServiceBreakdown(todayFilter),
      getServiceBreakdown(weekFilter),
      getServiceBreakdown(monthFilter),
      getServiceBreakdown(yearFilter),
      getServiceBreakdown(baseFilter)
    ]);

    // ================================================
    // 10. CALCULATE TOTAL COMMISSIONS
    // ================================================
      ...
      data: {
        ...
        periodStats: {
          today: {
            count: todayCount,
            amount: todayAmount,
            commission: todayAmount * 0.02,
            label: 'Today'
          },
          week: {
            count: weekCount,
            amount: weekAmount,
            commission: weekAmount * 0.02,
            label: 'This Week'
          },
          month: {
            count: monthCount,
            amount: monthAmount,
            commission: monthAmount * 0.02,
            label: 'This Month'
          },
          year: {
            count: yearCount,
            amount: yearAmount,
            commission: yearAmount * 0.02,
            label: 'This Year'
          }
        },
        ...
        transactions: transactions.map(tx => {
          ...
          return {
            ...
            serviceLabel: serviceCommissionRates[serviceType]?.label || 'Other',
    ...
    console.log(`   Today: ${todayCount} transactions, ₦${(todayAmount * 0.02).toFixed(2)} commission`);
    ...
    try {
      ...
      if (vtpassResult.success && vtpassResult.data && vtpassResult.data.code === '000') {
        ...
        try {
          await Notification.create({
            recipient: userId,
            title: isPackageChange ? 'TV Package Changed Successfully 📺' : 'TV Subscription Successful 📺',
            message: `Your ${serviceID.toUpperCase()} TV subscription${isPackageChange ? ' change from ' + packageChangeDetails.from + ' to ' + packageChangeDetails.to : ''} of ₦${totalAmount} for ${billersCode} was completed successfully. New wallet balance: ₦${newBalance}`,
            type: 'transaction',
            isRead: false,
            metadata: {
              serviceID: serviceID,
      ...
      // Handle duplicate request_id response from VTpass
        ...
        if (existingTx && existingTx.status === 'Successful') {
          ...
          return res.json({
            success: true,
            message: 'TV subscription already completed successfully',
            alreadyProcessed: true,
      ...
      const newTransaction = await createTransaction(
        ...
        transactionStatus,
        `${serviceID.toUpperCase()} subscription for ${billersCode}${isPackageChange ? ' - Package Change: ' + packageChangeDetails.from + ' → ' + packageChangeDetails.to : ''}${transactionStatus === 'Failed' ? ' - USER DEBITED' : ''}`,
        balanceBefore,
        balanceAfter,
        session,
        false,
...
app.post('/api/international-airtime/purchase', 
  ...
  userServiceRateLimiter('int_airtime', 3, 60000), // Max 3 international airtime purchases per minute
  [
  ...
  async (req, res) => {
    ...
    console.log('🌍 INTERNATIONAL AIRTIME PURCHASE REQUEST (RACE PROTECTED + IMMEDIATE DEBIT)');
    console.log('Request Body:', req.body);
    ...
    try {
      ...
      // 🔥 IMMEDIATE DEBIT ON PIN ENTRY - DEBIT NOW!
      // ================================================
      console.log(`🔒 IMMEDIATE DEBIT: User ${userId} - ${currency} ${amount} international airtime to ${phoneNumber}`);
      
      // Check balance first (convert to NGN for comparison)
      const amountInNGN = amount; // Assume amount is already in NGN for now
      
      if (user.walletBalance < amountInNGN) {
        await session.abortTransaction();
        return res.status(400).json({ 
          success: false, 
          message: `Insufficient balance. Required: ₦${amountInNGN}, Available: ₦${user.walletBalance.toFixed(2)}`,
          code: 'INSUFFICIENT_BALANCE'
        });
      }

      // ✅ DUPLICATE CHECK: Check for recent transaction to same phone number
      const thirtySecondsAgo = new Date(Date.now() - 30000);
      const existingTransaction = await Transaction.findOne({
        userId: userId,
      ...
      if (existingTransaction) {
        await session.abortTransaction();
        console.log(`🚫 DUPLICATE INTERNATIONAL AIRTIME BLOCKED: ${phoneNumber} - ${currency} ${amount}`);
        return res.status(409).json({
          success: false,
      ...
      // ================================================
      const balanceBefore = user.walletBalance;
      user.walletBalance -= amountInNGN;
      const balanceAfter = user.walletBalance;
      await user.save({ session });
      ...
      if (vtpassResult.success && vtpassResult.data?.code === '000') {
        ...
        try {
          await Notification.create({
            ...
            message: `International airtime of ${currency} ${amount} sent to ${phoneNumber} (${countryCode}). New balance: ₦${balanceAfter.toFixed(2)}`,
            type: 'transaction',
            isRead: false,
            metadata: {
        ...
        console.log(`✅ INTERNATIONAL AIRTIME SUCCESS: ${countryCode} - ${currency} ${amount} to ${phoneNumber}`);
        ...
        try {
          await Notification.create({
            recipient: userId,
            title: "International Airtime Purchase Issue ⚠️",
            message: `Your wallet was debited ₦${amountInNGN} for international airtime to ${phoneNumber}, but delivery failed. Our team will investigate.`,
            type: 'transaction_issue',
            isRead: false,
            metadata: {
              phoneNumber: phoneNumber,
      ...
      console.log(`✅ INTERNATIONAL AIRTIME TRANSACTION COMPLETE: ${countryCode} - ${currency} ${amount} to ${phoneNumber} - Status: ${transactionStatus} - User debited: true`);
      ...
      await session.abortTransaction();
      console.error('💥 INTERNATIONAL AIRTIME PURCHASE ERROR:', error);
